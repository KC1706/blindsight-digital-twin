...
| **Recommended action** | The driver-appropriate fix (labor move / tool recalibration / demand pacing), its projected effect, confidence, and Accept/Override | Found by re-simulating "what-if" changes. **Accept** shows a green confirmation with the projected units recovered/contained and writes the decision to the audit trail (visible to the plant manager); **Override** keeps the current plan and logs that. Decision-support only — nothing on the real line changes until a human acts. |
